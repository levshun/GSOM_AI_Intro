...
**Agent loop** – The repeating cycle of an agent: decide which tool to call → run it → read the result → repeat until the task is done.
...
**OpenCode** – An open-source agentic (command-line) assistant. It is a concrete, everyday example of an *agent*: it keeps a conversation, connects to a language model, and uses tools (reading files, running commands, editing files) to complete tasks.
